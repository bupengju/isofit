...
class NeuralNetworkRT:
    ...
    def __init__(self, config, instrument):
        ...
        for q in ['weights1_file', 'weights2_file', 
                  'biases1_file', 'biases2_file', 
                  'solar_file', 'wavelength_file']:
            if q not in config:
        ...
        # weights1.shape: (n_channels, n_atm, n_hidden1)
        # weights2.shape: (n_channels, n_hidden2, 1)
        self.weights1 = s.load(self.weights1_file)
        self.weights2 = s.load(self.weights2_file)
        
        # biases1.shape: (n_channels, n_hidden1)
        # biases2.shape: (n_channels, 1)
    ...
    def calc_rho(self, x_RT, rfl, Ls=None, geom=None):
        ...
        # Feedforward with (one or two) relu hidden and linear output layers
        ...
        rho = s.array(rho)
        if rho.shape[-1]!=1:
            n_wl,n_over = rho.shape[0],rho.shape[-1]

            n_half = n_over // 2
            rho_avg = s.zeros(n_wl)
            cinc = s.zeros(n_wl)
            for ci in range(n_wl):
                n_off = n_wl-ci
                if ci < n_half+1:
                    cmin,cmax = 0,n_over
                elif n_off < n_half+1:
                    cmin,cmax = n_wl-n_over,n_wl
                else:
                    cmin,cmax = ci-n_half,ci+n_half+1
                c = s.arange(cmin,cmax)
                cinc[c] += 1
                rho_avg[c] += rho[ci]
            rho = rho_avg / cinc
